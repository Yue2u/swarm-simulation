...
Статус: **Day 1 — done**, **Day 2 — done**, **Day 3 — done**, **Day 4 — done**.

## Статус на конец Дня 4

Сделано в День 4 (всё проверено `cargo test --workspace`):
* рельеф: `TerrainGpu` печёт `Rgba32Float` heightfield compute-проходом, biome mask выводится из
  тех же весов, что и палитра; меш рисуется vertex-pulling'ом одним draw'ом;
* три биома (forest/dunes/canyon) переключаются непрерывно через маску, а не скачком;
  scatter деревьев по blue-noise с учётом маски, меш генерится на CPU;
* атмосфера: аналитическое одиночное рассеяние Rayleigh + Mie, `aerial_perspective` для
  дальнего тумана, диск солнца; палитры птиц завязаны на биом;
* **исправлено:** меш рельефа собирался из конфига *активного* мира, поэтому при старте в
  подводном мире и переключении в небо рисовался рельеф размера рыбьей сцены. Теперь рельеф
  строится для неба всегда (рыбы heightfield не читают);
* **исправлено:** половина треугольников рельефа имела обратную намотку и отсекалась
  back-face culling'ом — земля рисовалась сеткой, сквозь которую видно небо. Тест
  `render/terrain_grounds_the_sky_world` держит это;
* **исправлено:** slope-терм в `render/terrain.wgsl` был инвертирован, из-за чего ровная земля
  красилась в голый камень; теперь работает как задумано;
* **исправлено:** стая распадалась на мелкие группы. Две причины: спавн раздавал агентам
  случайные направления (плотный кластер разлетался баллистически, пока alignment/cohesion его
  не соберут), и адаптивные веса (`w_sep *= 1+density`, `w_coh *= exp(-density)`) были уже
  ~10x separation-доминантны при `density_ref`. Теперь спавн когерентный (общее направление +
  `spawn_spread`), а веса симметричны вокруг `density_ref`: `push = density^density_gain`,
  `w_sep *= push`, `w_coh /= push`. Плюс `density_ref` поднят с 12 до 30 — средняя степень
  (`= density_ref`) обязана превышать порог связности случайного геометрического графа `ln n`
  (~11.5 при 100k), иначе любой провал плотности рвёт флок. Проверено на GPU при 100k: птицы —
  одна компонента (было 122), рыбы — 9 при 98% в крупнейшей; тест
  `spawn/spawn_graph_is_connected_at_scale` держит порог, `reference/spawned_flock_stays_one_group`
  — динамику;
* **производительность рыб:** подводный raymarch теперь идёт в half-res таргет, а
  `render/ocean_resolve.wgsl` апсемплит его в полный кадр и восстанавливает `frag_depth` из
  линейной дистанции, записанной в alpha. Это ~4x меньше пикселей у самого дорогого прохода;
* **морфинг Fish <-> Birds за 1.5 с:** `mesh_profile::scene_uniform_morph` лерпит `MeshParams`
  (форма, анимация, светимость, палитра) от одного мира к другому, `SceneUniform.mesh.variant`
  ведёт непрерывные ветки крыла и анимации, а `mesh.medium` берётся у целевого мира, чтобы агент
  не тонул в in-scatter среды, которую уже покинул. Переключение по TAB не трогает device/surface.

Не сделано:
* морфинг окружения, а не только агентов: океан и небо — это разная геометрия на разных
  масштабах, поэтому окружение меняется в кадр нажатия, а кросс-фейдятся только тела. Полный
  dissolve двух миров потребовал бы рендера обоих окружений и смешивания, что не стоит 1.5 с.
...
* `FishMode`: sphere trace по тому же `reef_field`, что и симуляция, теперь в half-res таргет;
  `render/ocean_resolve.wgsl` апсемплит его в полный кадр и пишет `frag_depth`, поэтому рыбы
  перекрываются рифом; Beer-Lambert по каналам, каустика, god rays, биолюминесценция
...
* коллизии птиц с рельефом: поле `EnvironmentKind::Terrain` и меш теперь из одного поля, но
  отдельного теста на то, что птица не проходит сквозь нарисованный склон, пока нет.
...
3. **Не сломать существующее.** `spawn_swarm` используют app, screenshot и обе стороны CPU/GPU-сверки;
   ...
   меньше порога).
4. **Стая снова распадалась (День 4).** После исправления раскладки рой всё ещё рвался на мелкие
   группы, а в группах агенты слетались слишком близко. Корень был не в кластере, а в двух других
   местах: `spawn_swarm` давал каждому агенту случайное направление (плотный шар разлетался
   баллистически быстрее, чем alignment успевал его собрать), а адаптивные веса были ~10x
   separation-доминантны при `density_ref`, то есть кластер был «заряжен» на разрыв; при этом
   низкая плотность отщепившихся групп усиливала cohesion обратно пропорционально и сталкивала их.
   Исправлено: когерентный спавн (`spawn_spread`), симметричные веса `push = density^density_gain`,
   и `density_ref = 30` ради связности графа восприятия при 100k. См. статус Дня 4.
